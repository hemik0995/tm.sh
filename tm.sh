...
tgid="-736626913# ID чата для автобэкапа (Необходим доступ к сообщениям)
tg2id="-736626913# ID чата диллерской группы для отправки ключей
admls="5550571460"	   # ID Админа
bot_api="5468064902:AAH0NoJIHNNJZjWVUyGJWZ_akQNOKGDRXg8" # Токен бота 
	...
	if [[ "$lalal" == "install" ]]; then
		...
		do
			...
			if [[ -z "${match_add}" ]]; then
				...
				ip=$(cat ${config_user_api_file}|grep "SERVER_PUB_ADDR = "|awk -F "[']" '{print $2}')
				ss_ssr_determine
				curl -s -X POST https://api.telegram.org/bot"$bot_api"/sendMessage -d chat_id="$tg2id" -d parse_mode=Markdown -d text="Был создан ключ Shadowsocks %0A Сервер: $backup_serv_id %0A Никнейм: ${ssr_user} %0A Port: ${ssr_port} %0A Дата удаления: ${deldate} %0A Ключ: ${tg_ss_link}" >> curl.tmp
				rm -r curl.tmp
...
${Purple}|${Font_default}${Red}Düzen:${Yellow} Stefano ${Purple}                            ${Font_default}${Purple}|${Font_default}
${Purple}|${Font_default}${Red}Telegram:${Yellow} @stefano ${Purple}                        ${Font_default}${Purple}|${Font_default}
